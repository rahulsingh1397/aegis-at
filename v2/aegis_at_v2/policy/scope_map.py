"""
policy/scope_map.py — static command->scope mapping (ported verbatim from v1).
...
Unknown commands raise ValueError (Rule 12: an unknown command is an
error, not a default). Adding a new command means adding it here,
deliberately, with a tested entry.
...
def scope_for_command(command: str) -> str:
    ...
    to score them, by design (Rule 12).
    """
    if command not in _SCOPE_REQUIRED:
        raise ValueError(
            f"unknown command: {command!r}. "
            f"Supported: {sorted(_SCOPE_REQUIRED)}. "
...
def known_commands() -> list[str]:
    """The set of supported commands. Useful for tests and tooling."""
